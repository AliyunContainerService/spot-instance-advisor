...
func (ms *MetaStore) Initialize(region string) error {
	req := ecsService.CreateDescribeInstanceTypesRequest()
	req.RegionId = region
	resp, err := ms.DescribeInstanceTypes(req)
	if err != nil {
		return errors.New(fmt.Sprintf("Failed to DescribeInstanceTypes,because: %s", err.Error()))
		//panic(fmt.Sprintf("Failed to DescribeInstanceTypes,because of %s", err.Error()))
	}
	...
	if err != nil {
		return errors.New(fmt.Sprintf("Failed to get available resource,because:  %s", err.Error()))
		//panic(fmt.Sprintf("Failed to get available resource,because of %s", err.Error()))
	}
	...
	for _, instanceType := range instanceTypes {
		...
		if err != nil {
			logger.WithFields(logger.Fields{
				"resp": resp,
				"err":  err,
			}).Errorf("describe spot price history error is :")
			continue
		}
